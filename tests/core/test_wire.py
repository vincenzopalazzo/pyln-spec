...
def test_primitives():
    raw_privkey = unhexlify(
        "1111111111111111111111111111111111111111111111111111111111111111"
    )
    privkey = PrivateKey(raw_privkey)
    pubkey = privkey.public_key()
    assert (
        hexlify(pubkey.serializeCompressed())
...
def test_handshake():
    ...
    rs_privkey = PrivateKey(
        unhexlify("2121212121212121212121212121212121212121212121212121212121212121")
    )
    rs_pubkey = rs_privkey.public_key()
    assert (
        hexlify(rs_pubkey.serializeCompressed())
        == b"028d7500dd4c12685d1f568b4c2b5048e8534b873319f3a8daa612b469132ec7f7"
    )
    ...
    assert (
        hexlify(ls_pubkey.serializeCompressed())
        == b"034f355bdcb7cc0af728ef3cceb9615d90684bb5b2ca5f859ab0f0b704075871aa"
    )
    ...
    lc1.handshake["e"] = PrivateKey(
        unhexlify("1212121212121212121212121212121212121212121212121212121212121212")
    )
    lc2.handshake["e"] = PrivateKey(
        unhexlify(b"2222222222222222222222222222222222222222222222222222222222222222")
    )
    assert (
        hexlify(lc1.handshake["e"].public_key().serializeCompressed())
        == b"036360e856310ce5d294e8be33fc807077dc56ac80d95d9cd4ddbd21325eff73f7"
    )

    assert (
        hexlify(lc1.handshake["h"])
        == b"8401b3fdcaaa710b5405400536a3d5fd7792fe8e7fe29cd8b687216fe323ecbd"
    )
    assert lc1.handshake["h"] == lc2.handshake["h"]
    m = lc1.handshake_act_one_initiator()
    assert (
        hexlify(m)
        == b"00036360e856310ce5d294e8be33fc807077dc56ac80d95d9cd4ddbd21325eff73f70df6086551151f58b8afe6c195782c6a"
    )

    lc2.handshake_act_one_responder(m)
    assert (
        hexlify(lc1.handshake["h"])
        == b"9d1ffbb639e7e20021d9259491dc7b160aab270fb1339ef135053f6f2cebe9ce"
    )
    assert hexlify(lc1.handshake["h"]) == hexlify(lc2.handshake["h"])

    assert (
        hexlify(lc1.chaining_key)
        == b"b61ec1191326fa240decc9564369dbb3ae2b34341d1e11ad64ed89f89180582f"
    )
    assert (
        hexlify(lc2.chaining_key)
        == b"b61ec1191326fa240decc9564369dbb3ae2b34341d1e11ad64ed89f89180582f"
    )
    ...
    assert (
        hexlify(m)
        == b"0002466d7fcae563e5cb09a0d1870bb580344804617879a14949cf22285f1bae3f276e2470b93aac583c9ef6eafca3f730ae"
    )
    assert (
        hexlify(lc2.handshake["h"])
        == b"90578e247e98674e661013da3c5c1ca6a8c8f48c90b485c0dfa1494e23d56d72"
    )
    ...
    assert (
        hexlify(lc1.chaining_key)
        == b"e89d31033a1b6bf68c07d22e08ea4d7884646c4b60a9528598ccb4ee2c8f56ba"
    )
    assert (
        hexlify(lc2.chaining_key)
        == b"e89d31033a1b6bf68c07d22e08ea4d7884646c4b60a9528598ccb4ee2c8f56ba"
    ...
    assert (
        hexlify(m)
        == b"00b9e3a702e93e3a9948c2ed6e5fd7590a6e1c3a0344cfc9d5b57357049aa22355361aa02e55a8fc28fef5bd6d71ad0c38228dc68b1c466263b47fdf31e560e139ba"
    )
    assert (
        hexlify(lc1.sk)
        == b"969ab31b4d288cedf6218839b27a3e2140827047f2c0f01bf5c04435d43511a9"
    )
    assert (
        hexlify(lc1.rk)
        == b"bb9020b8965f4df047e07f955f3c4b88418984aadc5cdb35096b9ea8fa5c3442"
    )
    ...
    assert (
        hexlify(lc1.chaining_key)
        == b"919219dbb2920afa8db80f9a51787a840bcf111ed8d588caf9ab4be716e42b01"
    )
...
def test_read_key_rotation():
    ...
    assert (
        hexlify(m)
        == b"cf2b30ddf0cf3f80e7c35a6e6730b59fe802473180f396d88a8fb0db8cbcf25d2f214cf9ea1d95"
    )
    ...
    assert (
        hexlify(m)
        == b"178cb9d7387190fa34db9c2d50027d21793c9bc2d40b1e14dcf30ebeeeb220f48364f7a4c68bf8"
    )
    ...
    assert (
        hexlify(m)
        == b"1b186c57d44eb6de4c057c49940d79bb838a145cb528d6e8fd26dbe50a60ca2c104b56b60e45bd"
    )
    ...
    assert (
        hexlify(m)
        == b"4a2f3cc3b5e78ddb83dcb426d9863d9d9a723b0337c89dd0b005d89f8d3c05c52b76b29b740f09"
    )
