#! /usr/bin/python3
from pyln_spec.core.message.fundamental_types import byte, u16, short_channel_id
from pyln_spec.core.message.array_types import (
    SizedArrayType,
    DynamicArrayType,
    EllipsisArrayType,
    LengthFieldType,
...
def test_sized_array():
    ...
    for arrtype, s, b in [
        [
            SizedArrayType(dummy("test1"), "test_arr", byte, 4),
            "00010203",
            bytes([0, 1, 2, 3]),
        ],
        [
...
def test_ellipsis_array():
    ...
    for arrtype, s, b in [
        [
            EllipsisArrayType(dummy("test1"), "test_arr", byte),
            "00010203",
            bytes([0, 1, 2, 3]),
        ],
        [
...
def test_dynamic_array():
    ...
    for arrtype, s, b in [
        [
            DynamicArrayType(dummy("test1"), "test_arr", byte, lenfield),
            "00010203",
            bytes([0, 1, 2, 3]),
        ],
        [
            DynamicArrayType(dummy("test2"), "test_arr", u16, lenfield),
            "[0,1,2,256]",
            bytes([0, 0, 0, 1, 0, 2, 1, 0]),
        ],
        [
            DynamicArrayType(dummy("test3"), "test_arr", short_channel_id, lenfield),
            "[1x2x3,4x5x6,7x8x9,10x11x12]",
            bytes(
                [0, 0, 1, 0, 0, 2, 0, 3]
                + [0, 0, 4, 0, 0, 5, 0, 6]
