#! /usr/bin/python3
from pyln_spec.core.message import MessageNamespace, Message
import pytest
import io
...
def test_static_array():
    ...
    ns.load_csv(["msgtype,test2,2", "msgdata,test2,test_arr,short_channel_id,4"])

    for test in [
        ["test1 test_arr=00010203", bytes([0, 1] + [0, 1, 2, 3])],
        [
...
def test_tlv():
    ...
    for test in [
        ...
        [
            ...
            bytes(
                [0, 1] + [1, 8, 1, 2, 3, 4, 0, 0, 0, 5] + [253, 0, 255, 4, 1, 2, 3, 4]
            ),
        ],
        [
...
def test_message_constructor():
    ...
    m = Message(
        ns.get_msgtype("test1"),
        tlvs="{tlv1={field1=01020304,field2=5}" ",tlv2={field3=01020304},4=010203}",
    )
    buf = io.BytesIO()
    m.write(buf)
    assert buf.getvalue() == bytes(
...
def test_dynamic_array():
    ...
    with pytest.raises(ValueError, match="Inconsistent length.*count"):
        m = Message(ns.get_msgtype("test1"), arr1="01020304", arr2="[1,2,3,4,5]")
