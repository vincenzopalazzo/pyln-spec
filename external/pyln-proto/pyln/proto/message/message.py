...
class MessageTypeField(object):
    ...
    def missing_fields(self, fieldvals: Dict[str, Any]):
        ...
        if (
            self.name not in fieldvals
            and not self.option
            and not self.fieldtype.is_optional()
        ):
            return [self]
        return []
...
class SubtypeType(FieldType):
    ...
    def len_fields_bad(self, fieldname: str, otherfields: Dict[str, Any]) -> List[str]:
        bad_fields: List[str] = []
        for f in self.fields:
            bad_fields += f.len_fields_bad(
                "{}.{}".format(fieldname, f.name), otherfields
            )
    ...
    def subtype_from_csv(parts: List[str]) -> "SubtypeType":
        ...
        if len(parts) != 1:
            raise ValueError("subtype expected 2 CSV parts, not {}".format(parts))
        return SubtypeType(parts[0])

    def _field_from_csv(
        self, namespace: MessageNamespace, parts: List[str], option: str = None
    ) -> MessageTypeField:
        ...
        if parts[2] != "":
            ...
            if lenfield is not None:
                ...
                field = MessageTypeField(
                    self.name,
                    parts[0],
                    DynamicArrayType(self, parts[0], basetype, lenfield),
                    option,
                )
                ...
                field = MessageTypeField(
                    self.name,
                    parts[0],
                    EllipsisArrayType(self, parts[0], basetype),
                    option,
                )
            else:
                field = MessageTypeField(
                    self.name,
                    parts[0],
                    SizedArrayType(self, parts[0], basetype, int(parts[2])),
                    option,
                )
        else:
    ...
    def val_from_str(self, s: str) -> Tuple[Dict[str, Any], str]:
        if not s.startswith("{"):
            raise ValueError(
                "subtype {} must be wrapped in '{{}}': bad {}".format(self, s)
            )
        s = s[1:]
        ret: Dict[str, Any] = {}
        ...
        while not s.startswith("}"):
            ...
            if f is None:
                raise ValueError(
                    "Unknown field name '{}'. Expected one of [{}])".format(
                        fieldname, ",".join(str(f) for f in self.fields)
                    )
                )
    ...
    def val_to_py(
        self, val: Dict[str, Any], otherfields: Dict[str, Any]
    ) -> Dict[str, Any]:
        ret: Dict[str, Any] = {}
    ...
    def write(
        self, io_out: BufferedIOBase, v: Dict[str, Any], otherfields: Dict[str, Any]
    ) -> None:
        self._raise_if_badvals(v)
        for f in self.fields:
            if f.name in v:
    ...
    def read(
        self, io_in: BufferedIOBase, otherfields: Dict[str, Any]
    ...
    def subfield_from_csv(namespace: MessageNamespace, parts: List[str]) -> None:
        ...
        if len(parts) != 4:
            raise ValueError("subtypedata expected 4 CSV parts, not {}".format(parts))
        ...
        if field.fieldtype.only_at_tlv_end():
            raise ValueError("{}: cannot have TLV field {}".format(subtype, field))
...
class MessageType(SubtypeType):
    ...
    onion_types = {"BADONION": 0x8000, "PERM": 0x4000, "NODE": 0x2000, "UPDATE": 0x1000}
    ...
    def msgtype_from_csv(parts: List[str]) -> "MessageType":
        ...
        if len(parts) == 3:
            ...
            raise ValueError("msgtype expected 3 CSV parts, not {}".format(parts))
    ...
    def msgfield_from_csv(namespace: MessageNamespace, parts: List[str]) -> None:
        ...
        if len(parts) == 5:
            ...
            raise ValueError("msgdata expected 4 CSV parts, not {}".format(parts))
        messagetype = namespace.get_msgtype(parts[0])
        if messagetype is None:
            raise ValueError("unknown subtype {}".format(parts[0]))

        field = messagetype._field_from_csv(namespace, parts[1:4], option=option)
...
class TlvStreamType(FieldType):
    ...
    def tlvtype_from_csv(namespace: MessageNamespace, parts: List[str]) -> None:
        ...
        if len(parts) != 3:
            raise ValueError("tlvtype expected 4 CSV parts, not {}".format(parts))
    ...
    def tlvfield_from_csv(namespace: MessageNamespace, parts: List[str]) -> None:
        ...
        """
        if len(parts) != 5:
            raise ValueError("tlvdata expected 6 CSV parts, not {}".format(parts))
        ...
        if field is None:
            raise ValueError("Unknown tlv field {}.{}".format(tlvstream, parts[1]))
    ...
    def val_from_str(self, s: str) -> Tuple[Dict[str, Any], str]:
        ...
        if not s.startswith("{"):
            raise ValueError(
                "tlvtype {} must be wrapped in '{{}}': bad {}".format(self, s)
            )
        s = s[1:]
        ret: Dict[str, Any] = {}
        while not s.startswith("}"):
    ...
    def write(
        ...
        def write_raw_val(
            iobuf: BufferedIOBase, val: Any, otherfields: Dict[str, Any]
        ) -> None:
            iobuf.write(val)
        ...
        ordered: List[
            Tuple[int, Callable[[BufferedIOBase, Any, Dict[str, Any]], None], Any]
        ] = []
        for fieldname in v:
            f = self.find_field(fieldname)
            if f is None:
    ...
    def read(
        ...
        while True:
            ...
            if len(binval) != tlv_len:
                raise ValueError("{}: truncated tlv {} value".format(tlv_type, self))
            ...
            if f is None:
                ...
                vals[f.name] = f.read(
                    cast(BufferedIOBase, BytesIO(binval)), otherfields
                )
...
class Message(object):
    ...
    def __init__(self, messagetype: MessageType, **kwargs):
        ...
        bad_lens = self.messagetype.len_fields_bad(self.messagetype.name, self.fields)
        if bad_lens:
    ...
    def set_field(self, field: str, val: Any) -> None:
        ...
        if isinstance(val, str):
            ...
            if remainder != "":
                raise ValueError(
                    "Unexpected {} at end of initializer for {}".format(
                        remainder, field
                    )
                )
        ...
        for f in mtype.fields:
            ...
            if fields[f.name] is None:
                ...
                if f.option is not None:
                    del fields[f.name]
                    break
                raise ValueError("{}: truncated at field {}".format(mtype, f.name))
    ...
    def write(self, io_out: BufferedIOBase) -> None:
        ...
        if self.missing_fields():
            raise ValueError("Missing fields: {}".format(self.missing_fields()))
