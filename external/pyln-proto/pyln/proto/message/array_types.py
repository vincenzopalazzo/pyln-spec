...
class ArrayType(FieldType):
    ...
    def val_from_str(self, s: str) -> Tuple[List[Any], str]:
        ...
        if not s.startswith("["):
            raise ValueError(
                "array of {} must be wrapped in '[]': bad {}".format(
                    self.elemtype.name, s
                )
            )
        s = s[1:]
        ret = []
    ...
    def write(
        self, io_out: BufferedIOBase, vals: List[Any], otherfields: Dict[str, Any]
    ) -> None:
...
class SizedArrayType(ArrayType):
    ...
    def __init__(
        self, outer: "SubtypeType", name: str, elemtype: FieldType, arraysize: int
    ):
    ...
    def write(
        self, io_out: BufferedIOBase, v: List[Any], otherfields: Dict[str, Any]
    ) -> None:
        if len(v) != self.arraysize:
            raise ValueError("Length of {} != {}", v, self.arraysize)
...
class LengthFieldType(FieldType):
    ...
    def __init__(self, inttype: IntegerType):
        if type(inttype) is not IntegerType:
            raise ValueError("{} cannot be a length; not an integer!".format(self.name))
    ...
    def calc_value(self, otherfields: Dict[str, Any]) -> int:
        ...
        if self.len_fields_bad("", otherfields):
            raise ValueError("Lengths of fields {} not equal!".format(self.len_for))
    ...
    def val_to_str(self, _, otherfields: Dict[str, Any]) -> str:
        return self.underlying_type.val_to_str(
            self.calc_value(otherfields), otherfields
        )

    def val_to_py(self, v: Any, otherfields: Dict[str, Any]) -> int:
        """Convert to a python object: for integer fields, this means an int"""
        return self.underlying_type.val_to_py(self.calc_value(otherfields), otherfields)
    ...
    def write(self, io_out: BufferedIOBase, _, otherfields: Dict[str, Any]) -> None:
        self.underlying_type.write(io_out, self.calc_value(otherfields), otherfields)
...
class DynamicArrayType(ArrayType):
    ...
    def read(self, io_in: BufferedIOBase, otherfields: Dict[str, Any]) -> List[Any]:
        return super().read_arr(
            io_in,
            otherfields,
            cast(LengthFieldType, self.lenfield.fieldtype)._maybe_calc_value(
                self.lenfield.name, otherfields
            ),
        )
