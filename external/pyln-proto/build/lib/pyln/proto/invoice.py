...
class Invoice(object):
    ...
    def decode(cls, b):
        ...
        if inv.pubkey:  # Specified by `n`
            ...
            inv.signature = coincurve.ecdsa.deserialize_recoverable(sigdecoded[0:65])
            inv.pubkey = coincurve.PublicKey.from_signature_and_message(
                sigdecoded[0:65], bytearray([ord(c) for c in hrp]) + data.tobytes()
            )
...
class RouteHint(object):
    ...
    def from_bytes(cls, b):
        ...
        (
            inst.fee_base_msat,
            inst.fee_proportional_millionths,
            inst.cltv_expiry_delta,
        ) = struct.unpack("!IIH", b.read(10))
        return inst
...
class RouteHintSet(object):
    ...
    def from_bytes(cls, b):
        ...
        if not isinstance(b, BufferedReader):
            raise TypeError(
                "from_bytes can only read from bytes-arrays or BufferedReader"
            )

        if len(b.raw.getvalue()) % RouteHint.length != 0:
            raise TypeError(
                "byte string is not a multiple of the route hint size: {}".format(
                    len(b.raw.getvalue())
                )
            )
