...
def try_unpack(
    name: str, io_out: BufferedIOBase, structfmt: str, empty_ok: bool
) -> Optional[int]:
...
class FieldType(object):
    ...
    def write(
        self, io_out: BufferedIOBase, v: Any, otherfields: Dict[str, Any]
    ) -> None:
        raise NotImplementedError()
...
class IntegerType(FieldType):
    ...
    def write(
        self, io_out: BufferedIOBase, v: int, otherfields: Dict[str, Any]
    ) -> None:
...
class ShortChannelIDType(IntegerType):
    ...
    def val_from_str(self, s: str) -> Tuple[int, str]:
        ...
        return ((int(parts[0]) << 40) | (int(parts[1]) << 16) | (int(parts[2]))), b
...
class TruncatedIntType(FieldType):
    ...
    def val_from_str(self, s: str) -> Tuple[int, str]:
        ...
        if int(a) >= (1 << (self.maxbytes * 8)):
            raise ValueError("{} exceeds maximum {} capacity".format(a, self.name))
        return int(a), b
    ...
    def write(
        ...
        if len(binval) > self.maxbytes:
            raise ValueError("{} exceeds maximum {} capacity".format(v, self.name))
        io_out.write(binval)
    ...
    def read(self, io_in: BufferedIOBase, otherfields: Dict[str, Any]) -> None:
        ...
        if len(binval) > 0 and binval[0] == 0:
            raise ValueError(
                "{} encoding is not minimal: {}".format(self.name, binval.hex())
            )
...
class FundamentalHexType(FieldType):
    ...
    def write(
        self, io_out: BufferedIOBase, v: bytes, otherfields: Dict[str, Any]
    ) -> None:
        if len(bytes(v)) != self.bytelen:
            raise ValueError("Length of {} != {}", v, self.bytelen)
        io_out.write(v)
