...
class ShortChannelId(object):
    ...
    def from_str(self, s):
        ...
        return ShortChannelId(block=int(block), txnum=int(txnum), outnum=int(outnum))
...
class PublicKey(object):
    def __init__(self, innerkey):
        ...
        if isinstance(innerkey, bytes):
            ...
            raise ValueError("Key must either be bytes or coincurve.keys.PublicKey")
        self.key = innerkey
    ...
    def __str__(self):
        return "PublicKey[0x{}]".format(self.serializeCompressed().hex())
