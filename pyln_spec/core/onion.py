...
class OnionPayload(object):
    ...
    def to_bytes(self, include_prefix):
        raise ValueError(
            "OnionPayload is an abstract class, use "
...
class LegacyOnionPayload(OnionPayload):
    def __init__(
        self, amt_to_forward, outgoing_cltv_value, short_channel_id=None, padding=None
    ):
    ...
    def __str__(self):
        return (
            "LegacyOnionPayload[scid={self.short_channel_id}, "
            "amt_to_forward={self.amt_to_forward}, "
            "outgoing_cltv={self.outgoing_cltv_value}]"
        ).format(self=self)
    ...
    def from_bytes(cls, b, skip_length=False):
        ...
        while b.tell() < start + payload_length:
            ...
            if length is None:
                ...
                raise ValueError(
                    "Failed to parse TLV payload: value length "
...
class RoutingOnion(object):
    def __init__(
        self, version: int, ephemeralkey: PublicKey, payloads: bytes, hmac: bytes
    ):
    ...
    def from_bin(cls, b: bytes):
        ...
        assert len(payloads) == ROUTING_INFO_SIZE and len(hmac) == HMAC_SIZE
        return cls(
            version=version, ephemeralkey=ephemeralkey, payloads=payloads, hmac=hmac
        )
    ...
    def to_bin(self) -> bytes:
        ...
        return struct.pack("b", self.version) + ephkey + self.payloads + self.hmac
    ...
    def unwrap(
        ...
        h = hmac.HMAC(keys.mu, hashes.SHA256(), backend=default_backend())
        ...
        if hh != self.hmac:
            raise ValueError(
                "HMAC does not match, onion might have been "
...
def xor(a: Union[bytearray, memoryview], b: Union[bytearray, memoryview]) -> bytearray:
    assert len(a) == len(b)
    d = bytearray(len(a))
    xor_inplace(d, a, b)
    return d
...
class SphinxPath(object):
    ...
    def compile(self) -> RoutingOnion:
        ...
        nexthmac = bytes(32)
        for i, h, p in zip(
            range(len(self.hops)), reversed(self.hops), reversed(params)
        ):
            h.hmac = nexthmac
            shift_size = len(h)
            ...
            hh = hmac.HMAC(p.keys.mu, hashes.SHA256(), backend=default_backend())
            hh.update(buf)
    ...
    def get_hop_params(self) -> List[SphinxHopParam]:
        ...
        secret = ecdh(PrivateKey(self.session_key.data), self.hops[0].pubkey)
        sph = SphinxHopParam(
        ...
        for i, h in enumerate(self.hops[1:]):
            prev = params[-1]
            ek = blind_group_element(prev.ephemeralkey, prev.blind)
