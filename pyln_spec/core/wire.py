...
class LightningConnection(object):
    ...
    def handshake_act_one_responder(self, m):
        ...
        if v != 0:
            raise ValueError(
                "Unsupported handshake version {}, only version "
        ...
        try:
            ...
            ValueError(
                "Verification of tag failed, remote peer doesn't know " "our node ID."
            )
        h.update(c)
    ...
    def handshake_act_three_responder(self, m):
        ...
        self.chaining_key, self.temp_k3 = hkdf_two_keys(se.data, self.chaining_key)
    ...
    def read_message(self):
        with self.recv_lock:
            ...
            if len(lc) != 18:
                raise ValueError(
                    "Short read reading the message length: 18 != {}".format(len(lc))
                )
    ...
    def shake(self):
        if self.is_initiator:
            ...
            if len(m) != 50:
                raise ValueError(
                    "Short read from peer reading act2: 50 != {}".format(len(m))
                )
            ...
            if len(m) != 50:
                raise ValueError(
                    "Short read from peer reading act1: 50 != {}".format(len(m))
                )
            ...
            if len(m) != 66:
                raise ValueError(
                    "Short read from peer reading act3: 66 != {}".format(len(m))
                )
...
def connect(
    ...
    if isinstance(node_id, bytes) and len(node_id) == 33:
        ...
        raise ValueError("node_id must be either a 33 byte array, or a PublicKey")
    ...
    lconn = LightningConnection(conn, remote_pubkey, local_privkey, is_initiator=True)
    lconn.shake()
    return lconn
